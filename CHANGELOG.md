...
- **A timed-out telemetry POST no longer stops telemetry for the life of the process (qfg-y8je.1).** The sender's `HttpClient.Timeout` (30s) surfaces as a `TaskCanceledException`, which the reporter treated as a shutdown: the flush loop exited and no further telemetry was ever sent, so a single slow api-telemetry response (e.g. a backend stall) silently ended all telemetry. The reporter now ends the loop only when its own shutdown token is cancelled; a per-request cancellation is handled like any other failed POST (batch dropped, interval backs off, loop keeps ticking). No public API change, no new dependencies.
